...
int Displayer::mouse_down_button_ = GL_LEFT, Displayer::mouse_down_x_ = 0,
...
void Displayer::initCallBackFunctions() {
  ...
  {
    light_x_ -= 0.05f;
    std::cerr << light_x_ << "," << light_y_ << "," << light_z_ << std::endl;
  });
  bindKey('d', [](int, int)
  {
    light_x_ += 0.05f;
...
void Displayer::CallBackDisplay() {
  ...
  camera = origin + mesh_tools::Point3(cos(rotate_a_) * cos(rotate_b_),
                                       sin(rotate_b_),
                                       sin(rotate_a_) * cos(rotate_b_));
  gluLookAt(camera[0], camera[1], camera[2], origin[0], origin[1], origin[2], 0,
            1, 0);
...
void Displayer::CallBackMotion(int x, int y) {
  ...
  if (mouse_down_button_ == GLUT_LEFT_BUTTON) {
    auto camera_offset = mesh_tools::Point3(cos(rotate_a_) * cos(rotate_b_),
                                            sin(rotate_b_),
                                            sin(rotate_a_) * cos(rotate_b_));
    auto x_offset = mesh_tools::Point3(-sin(rotate_a_), 0, cos(rotate_a_));
    auto y_offset = x_offset.outer_product(camera_offset);
    origin = mouse_down_origin + x_offset * dx * modelSize * scale_times * 0.001
             + y_offset * dy * modelSize * scale_times * 0.001;
...
void DrawableObject::drawTriangle(const mesh_tools::Point3& point_a,
                                  const mesh_tools::Point3& point_b,
                                  const mesh_tools::Point3& point_c) const {
...
void DrawableObject::setColor(double red, double green, double blue,
                              double alpha) const {
  glColor4d(red, green, blue, alpha);
}

void DrawableObject::drawLine(const mesh_tools::Point3& point_a,
                              const mesh_tools::Point3& point_b) const {
