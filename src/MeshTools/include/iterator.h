...
public:
  ...
  explicit FacetIterator(Surface& surface)
    : surface_(surface),
      ...
      unique_id_(facet_no_ != INVALID_ID
                   ? surface_.facet_manager_.UniqueID(facet_no_)
                   : INVALID_ID) {
  }
...
public:
  ...
  explicit VertexIterator(Surface& surface)
    : surface_(surface),
      ...
      unique_id_(vertex_no_ != INVALID_ID
                   ? surface_.vertex_manager_.UniqueID(vertex_no_)
                   : INVALID_ID) {
  }
...
public:
  HalfedgeIterator opposite() const {
    ...
    return HalfedgeIterator(surface_,
                            surface_
                            .halfedge_manager_.HalfedgeAt(halfedge_no_).
                            opposite);
  }
  ...
  HalfedgeIterator prev() const {
    ...
    return HalfedgeIterator(surface_,
                            surface_
                            .halfedge_manager_.HalfedgeAt(halfedge_no_).prev);
  }
  ...
  HalfedgeIterator next() const {
    ...
    return HalfedgeIterator(surface_,
                            surface_
                            .halfedge_manager_.HalfedgeAt(halfedge_no_).next);
  }
  ...
  VertexIterator vertex() const {
    assert(IsValid());
    return VertexIterator(surface_,
                          surface_
                          .halfedge_manager_.HalfedgeAt(halfedge_no_).vertex);
  }

  FacetIterator facet() const {
    assert(IsValid());
    return FacetIterator(surface_,
                         surface_
                         .halfedge_manager_.HalfedgeAt(halfedge_no_).facet);
  }

  explicit HalfedgeIterator(Surface& surface)
    : surface_(surface),
      halfedge_no_(surface.halfedge_manager().ElementBegin()),
      unique_id_(halfedge_no_ != INVALID_ID
                   ? surface_.halfedge_manager_.UniqueID(halfedge_no_)
                   : INVALID_ID) {
  }
  ...
  HalfedgeIterator& operator++() {
    assert(IsValid());
    halfedge_no_ = surface_.halfedge_manager_.
                            GetNextElementPosition(halfedge_no_);
    unique_id_ = halfedge_no_ != INVALID_ID
                   ? surface_.halfedge_manager_.UniqueID(halfedge_no_)
                   : INVALID_ID;
    return *this;
