...
namespace mesh_tools
{
  ...
  {
  public:
    explicit SurfaceFile(std::string path):file_path_(std::move(path)){}
  ...
  {
  public:
    explicit ObjFile(const std::string &path):SurfaceFile(path) {
    }
    ...
    void SaveToFile(Surface &surface)override
    {
      ...
      for (VertexIterator i(surface, 0); !i.IsEnd(); ++i)
      {
        ...
        file_ << "v " << i.position()[0] << " " << i.position()[1] << " " << i.position()[2] << std::endl;
      }
