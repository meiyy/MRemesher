...
public:
  explicit Editor(Surface& surface)
    : surface_(surface) {
  }
  ...
  void RemoveFacet(unsigned int facet_no) const {
    ...
    unsigned int halfedge[3] = {
      facet.halfedge().halfedge_no(), facet.halfedge().next().halfedge_no(),
      facet.halfedge().prev().halfedge_no()
    };
    ...
    for (auto i : halfedge) {
      ...
      if (!hit.opposite().IsValid() && !hit.prev_around_vertex().IsValid()) {
        ...
        if (hit.opposite().IsValid()) {
          surface_.vertex_manager_.SetHalfedge(hit.vertex().vertex_no(),
                                               hit.next_around_vertex().
                                                   halfedge_no());
        } else {
          surface_.vertex_manager_.SetHalfedge(hit.vertex().vertex_no(),
                                               hit.prev_around_vertex().
                                                   halfedge_no());
        }
      }
    }
    ...
    for (auto i : halfedge) {
      ...
      if (hit.opposite().IsValid()) {
        surface_.halfedge_manager_.SetOpposite(hit.opposite().halfedge_no(),
                                               UINT_MAX);
      } else {
